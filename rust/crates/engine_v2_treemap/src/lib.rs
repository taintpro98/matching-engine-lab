//! Engine v2: TreeMap (Red-Black tree) based implementation.
//! Uses rb_tree::RBMap for ordered iteration (cheapest-first, FIFO).
...
impl Engine for EngineV2 {
    ...
    fn stats(&self) -> HashMap<String, String> {
        ...
        m.insert("engine".to_string(), "v2_treemap".to_string());
...
impl EngineV2 {
    ...
    fn match_orders(
        ...
        while can_continue(remaining_qty, remaining_budget, max_budget) && !self.book.is_empty() {
            let (key, mut order) = match self.book.pop_pair() {
                Some((k, o)) => (k, o),
                None => break,
            };
